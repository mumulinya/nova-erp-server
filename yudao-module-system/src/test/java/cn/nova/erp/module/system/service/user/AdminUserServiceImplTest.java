...
import cn.nova.erp.module.infra.api.config.ConfigApi;
import cn.nova.erp.module.infra.api.file.FileApi;
...
import cn.nova.erp.module.system.dal.dataobject.dept.UserPostDO;
...
import cn.nova.erp.module.system.dal.mysql.dept.UserPostMapper;
...
import static cn.nova.erp.module.system.service.user.AdminUserServiceImpl.USER_INIT_PASSWORD_KEY;
...
public class AdminUserServiceImplTest extends BaseDbUnitTest {
    ...
    public void before() {
        // mock 初始化密码
        when(configApi.getConfigValueByKey(USER_INIT_PASSWORD_KEY)).thenReturn("novayuanma");
    }
    ...
    public void testCreatUser_success() {
        ...
        when(passwordEncoder.encode(eq(reqVO.getPassword()))).thenReturn("novayuanma");
        ...
        assertEquals("novayuanma", user.getPassword());
    ...
    public void testImportUserList_02() {
        ...
        when(deptService.getDept(eq(dept.getId()))).thenReturn(dept);
        // mock passwordEncoder 的方法
        when(passwordEncoder.encode(eq("novayuanma"))).thenReturn("java");
